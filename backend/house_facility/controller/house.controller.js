...
import fs from "fs"
import houseSchemaModel from "../models/house.schema.js"
...
export const add = async (req, res) => {
    ...
    try {
        ...
        if (check) {
            ...
            var result = await houseSchemaModel.create(details)
            if(result)
            res.status(200).json({ status: "true", msg: "House registred successfuly" });
...
export const fetch = async (req, res) => {
    ...
    // res.send("working")
    try {
        let result = await houseSchemaModel.find(req.query)
        if (result.length != 0) {
...
export var deleteMess = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            if (matchPwd) {
                console.log(matchPwd)
                var del = await houseSchemaModel.deleteOne({ "_id": req.body._id })
                if (del) {
...
export var updateMess = async (req, res, next) => {
    ...
    try {
        ...
        var user = await houseSchemaModel.findOne(JSON.parse(req.body.condition_obj));

        if (user) {
            await houseSchemaModel.updateOne(JSON.parse(req.body.condition_obj), { $set: (JSON.parse(req.body.content_obj)) });
